...
cd '/media/amr/HDD/Work/Stimulation/+ve_Stimulation'
mkdir '/media/amr/HDD/Work/Stimulation/Data' 
for folder in *;do
	...
	mkdir /media/amr/HDD/Work/Stimulation/Data/${folder}
	pwd
	...
	for image in *.nii.gz;do
		...
		if    [[ "$dim1" -eq "200"  &&  "$dim4" -eq "1" ]]; then
			...
			imcp $image /media/amr/HDD/Work/Stimulation/Data/${folder}/Anat_${folder}
			...
			imcp $image /media/amr/HDD/Work/Stimulation/Data/${folder}/Stim_${folder}_${number}
		fi
	done
	fslmerge -t EPI_Average_${folder} Example_*
	fslmaths EPI_Average_${folder} -Tmean EPI_Average_${folder} 
	imrm Example_* 
	imcp EPI_Average_${folder} /media/amr/HDD/Work/Stimulation/Data/${folder}/ 
