#!/bin/bash
#This is a fast and dirty script to run on the MRI machine to investigate if there is activation or not
#after applying the stimultion
#There is no co-registration or anything and the statistical threshold is very weak


#Amr: Thu Mar 29 14:56:36 CEST 2018
#Task_based fMRI
#Take Augmentation script with you
#take the stimulation file and designs with you 

#-------------------------------How to use--------------------------------------------------
#The script needs 3 arguments
#$1 -> folder where all the studies are 
#$2 -> name of the study e.g A140318
#$3 -> number of the study in two digits format e.g 17, 09
#e.g ./Machine_Task_based.sh /media/amr/HDD/Nipype_Trial/ A120318 14
#-------------------------------------------------------------------------------------------

cd $1
pwd

#Deleting previous sessions
rm $2/*.brkhdr 
rm $2/*.img 
rm $2/*.mat 
rm $2/*.hdr 

#Convert 2dseq (bruker default ouput) to analyze format)
pvconv.pl $2 -outdir $2

folder=${2}_${3}
echo $folder

cd ${2}
#change analyze to compressed nifti
