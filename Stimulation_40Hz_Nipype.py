...
HighresToTemplate.inputs.convergence_window_size=[40]
HighresToTemplate.inputs.metric=['MI', 'MI', 'CC']
HighresToTemplate.inputs.metric_weight=[1.0]*3
HighresToTemplate.inputs.number_of_iterations=[[4000, 500, 250, 400],
                                                 [4000, 500, 250, 400],
                                                 [400, 70, 50, 20]]
...
CoReg.inputs.convergence_window_size=[40]
CoReg.inputs.metric=['MI', 'MI', 'CC']
CoReg.inputs.metric_weight=[1.0]*3
CoReg.inputs.number_of_iterations=[[4000, 500, 250, 400],
                                                 [4000, 500, 250, 400],
                                                 [400, 70, 50, 20]]
...
def Plot_Motion(motion_par, abs_disp, rel_disp):
    ...
    plt.figure(figsize=(8,40), dpi=300)
...
#Spm_Smoothing.iterables = ('fwhm', [[5,5,8],[5.75,5.75,8],[5.75,5.75,40], [5.75,5.75,16]])
...
def Scale_Median_Intensity (median_intensity):
    scaling = 40000/median_intensity
    return scaling
...
#the grand-mean intensity normalisation factor ( to give a median brain intensity of 40000 )
#grand mean scaling
...
Film_Gls.inputs.threshold = 4000.0
